...
def index(request,pIndex=1):
    '''浏览订单信息'''
    umod = Orders.objects
    sid = request.session['shopinfo']['id']#获取当前店铺id
    list = umod.filter(shop_id=sid)
    mywhere=[]
...
def insert(request):
    ...
    try:
        ...
        #遍历购物车中的菜品并添加到订单详情中
        for shop in cartlist.values():#shop为定义的变量
            ov = OrderDetail()
            ov.order_id = od.id
            ov.product_id = shop['id']#菜品
            ov.product_name = shop['name']
            ov.price = shop['price']
            ov.quantity = shop['num']
            ov.status = 1 #状态1正常/9删除
            ov.save()
        del request.session['cartlist']  
        del request.session['total_money']#买完就清空
